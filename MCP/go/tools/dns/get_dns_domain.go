...
func Get_dns_domainHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/dns/%s%s", cfg.BaseURL, domain, queryString)
...
func CreateGet_dns_domainTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_dns_domain",
		mcp.WithDescription("View all your records for a given domain"),
		mcp.WithString("domain", mcp.Required(), mcp.Description("Domain name you want to get records for")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_dns_domainHandler(cfg),
	}
}
